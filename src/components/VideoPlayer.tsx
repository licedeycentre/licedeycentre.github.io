...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({ 
  ...
  if (videos && videos.length > 0) {
    return (
      <div className={className}>
        {title && (
          <h3 className="video-group-title">{title}</h3>
        )}
        {videos.map((video, index) => {
          ...
          return (
            <div key={index} className={`video-player-wrapper ${isVideo ? 'video-player-wrapper--adaptive' : ''}`}>
              {youtubeId ? (
                <iframe
                  width="100%"
                  height="100%"
                ...
                <div style={{ background: '#f0f0f0', padding: '20px', textAlign: 'center', color: '#666' }}>
  ...
  return (
    <div className={className}>
      ...
      {description && (
        <p className="video-player-description">{description}</p>
      )}
