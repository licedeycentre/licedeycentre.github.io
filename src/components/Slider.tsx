...
import { formatShowDate, filterFutureShowDates, getLastPastShowDate, formatLastShowDate } from '../utils/dateFormat'
...
export const Slider: React.FC<SliderProps> = ({ 
  ...
  const current = mode === 'hero' 
    ? (slides.length > 0 ? slides[index] : fallbackSlide)
    : null
  ...
  const hasContent = () => {
    ...
    if (mode === 'gallery') {
      return index === 0 && !!(content.title || content.description || content.meta?.length || content.ticketsUrl || content.buttons?.length || content.showDates?.length)
    }
    
    // В режиме hero показываем контент всегда
    return !!(content.title || content.description || content.meta?.length || content.ticketsUrl || content.buttons?.length || content.showDates?.length)
  }
      ...
      <div className="hero-content">
        <div className="hero-text">
          ...
          {mode === 'gallery' && content.meta && content.meta.length > 0 && (
            <div className="hero-meta">
              {content.meta.map((item, i) => (
                <span key={i} className="hero-meta-item">{item}</span>
              ))}
            </div>
          )}
          ...
          <div className="hero-buttons">
            ...
            {mode === 'hero' && content.buttons?.map((button, i) => (
              <Link
                key={i}
                to={button.href}
            ...
            {mode === 'gallery' && content.ticketsUrl && (() => {
  ...
  const renderNavigation = () => {
    ...
    if (navigationTotal <= 1) return null
    
    const ariaLabel = mode === 'hero' 
      ? "Навигация слайдера главной страницы"
      : "Навигация слайдера спектакля"
      ...
      <div className="hero-nav" aria-label={ariaLabel}>
        ...
        >
          <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true">
            <path d="M15.5 4.5L8 12l7.5 7.5" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
        <button 
          type="button" 
        ...
        >
          <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true">
            <path d="M8.5 4.5L16 12l-7.5 7.5" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
      </div>
    )
  ...
  const displayImages = mode === 'gallery' && images.length === 0 
