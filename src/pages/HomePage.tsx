      ...
      <>
          ...
          <div className="container">
            <div className="section-header-with-nav">
              <h2 className="link-title">
                <Link to="/publications" className="section-title-link">Публикации</Link>
              </h2>
            </div>
        ...
        <section className="section section--compact">
          <div className="container">
            <div className="section-header-with-nav">
              <h2 className="link-title">
                <Link to="/performances" className="section-title-link">Спектакли</Link>
              </h2>
            </div>
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--muted)' }}>
          ...
          {pubPreview.length > 0 && (
            <>
              <div className="publications-grid">
                {pubPreview.map((post) => (
                  <Link key={generatePublicationId(post.title, post.date)} to={`/publications/${generatePublicationId(post.title, post.date)}`} className="publication-card">
                    <MediaThumb 
                      src={post.image} 
                      alt={post.title} 
                      ...
                      {post.tags && post.tags.length > 0 && (
                        <div className="card-meta">
                          {post.tags.map((tag: string, i: number) => (
                            <span key={i} className="card-meta-badge">{tag}</span>
                          ))}
                        </div>
                      )}
                      {post.date && (
